...
export default defineConfig(({ mode }) => ({
  ...
  resolve: {
    alias: {
      '@/admin/loadAdminApp': path.resolve(
        __dirname,
        mode.startsWith('desktop') ? './src/admin/DesktopAdminStub.tsx' : './src/admin/loadAdminApp.ts'
      ),
        ...
        manualChunks: (id) => {
          ...
          if (!id.includes('node_modules')) {
            if (id.includes('/pages/')) {
              ...
              // Split the large pages-other chunk into smaller pieces
              if (id.includes('Attendance.tsx') || id.includes('Transport.tsx') || id.includes('Boarding.tsx')) return 'pages-daily';
              if (id.includes('Classes.tsx') || id.includes('ClassDetail.tsx') || id.includes('Subjects.tsx') || id.includes('Timetable.tsx')) return 'pages-academic-mgmt';
              if (id.includes('Settings.tsx') || id.includes('Plans.tsx') || id.includes('Subscription.tsx')) return 'pages-settings';
              if (id.includes('Notifications.tsx') || id.includes('Announcements.tsx') || id.includes('RecycleBin.tsx')) return 'pages-comms';
              return 'pages-other';
            }
            if (id.includes('/admin/')) return 'admin';
            return undefined;
          }
          // Vendor chunking — most important: keep react + router in first load
          if (id.includes('node_modules/react/') || id.includes('node_modules/react-dom/') || id.includes('node_modules/scheduler/')) return 'vendor-react';
          if (id.includes('react-router')) return 'vendor-router';
          if (id.includes('@tanstack')) return 'vendor-query';
          if (id.includes('lucide-react')) return 'vendor-icons';
          if (id.includes('dompurify')) return 'vendor-sanitize';
          // Heavy on-demand vendors — loaded lazily via dynamic import pages
